...
with open(CITY_FILE, newline='') as csvfile:
    ...
    for row in spamreader:
        ...
        formal_name = ""
        if row[1] != '': 
            formal_name = format_name(row[1])
            set_name.add(formal_name)
        if row[2] != '': 
            formal_name = format_name(row[2])
            set_name.add(formal_name)

        set_alter_name = set()
        if row[3] != '': 
            set_alter_name.update([format_name(x) for x in row[3].split(',')])
        set_alter_name.difference(set_blackwords)
        set_alter_name = {x for x in set_alter_name if is_good_name(x)}

        # build the mapping from alter name to name
        set_name.update(set_alter_name)
        set_name.difference(set_blackwords)
        set_name = {x for x in set_name if is_good_name(x)}

        for name in set_name:
            dict_city_alter_to_name[name] = formal_name
        ...
        for name in set_name:
            ...
            # add to city name
            # TODO: bad process, should mark the alter city name and their origin name 
...
pickle.dump(dict_city_alter_to_name, open(f'{BIN_DIR}/dict_city_alter_to_name.bin', 'wb'))